...
// Float64s converts uint64 slice to float32 slice.
...
func (u Uint64Slice) Ints() ([]int, error) {
	r := make([]int, len(u))
	if is64BitPlatform {
		for k, v := range u {
			if v > math.MaxInt64 {
...
// Uint64s converts uint64 slice to uint16 slice.
...
// Uint64s converts uint64 slice to uint32 slice.
...
func (u Uint64Slice) Reduce(
	fn func(curr Uint64Slice, k int, v uint64, accumulator uint64) uint64, initialValue ...uint64,
) uint64 {
	if len(u) == 0 {
		return 0
	}
